...
function VentasView({ user }) {
  ...
  return (
    <div className="space-y-6" data-testid="ventas-view">
      ...
      {/* Detail Modal */}
      <Modal isOpen={showDetailModal} onClose={() => setShowDetailModal(false)} title="Detalle de Venta" size="lg">
        {selectedVenta && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div><span className="text-muted-foreground">Cliente:</span> <span className="font-medium">{selectedVenta.cliente_nombre}</span></div>
              <div><span className="text-muted-foreground">Fecha:</span> <span className="font-medium">{new Date(selectedVenta.fecha).toLocaleDateString('es-PY')}</span></div>
            </div>
            <table className="data-table">
              <thead><tr><th>Producto</th><th className="text-center">Cant.</th><th className="text-right">Precio</th><th className="text-right">Subtotal</th></tr></thead>
              <tbody>
                {(selectedVenta.items || []).map((item, i) => (
                  <tr key={i}>
                    ...
                    <td className="text-right">{formatGs(item.precio_unitario)}</td>
                    <td className="text-right font-medium">{formatGs(item.precio_unitario * item.cantidad)}</td>
                  </tr>
                ))}
                <tr className="font-semibold bg-muted">
                  <td colSpan={3} className="text-right">TOTAL:</td>
                  <td className="text-right text-lg">{formatGs(selectedVenta.total)}</td>
                </tr>
              </tbody>
            </table>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editItem ? 'Editar Venta' : 'Nueva Venta'} size="xl">
        <div className="space-y-4">
          ...
          <div className="border border-border rounded-lg p-4">
            ...
            {form.items.length > 0 && (
              <table className="data-table">
                <thead><tr><th>Producto</th><th className="text-center w-24">Cant.</th><th className="text-right w-36">Precio</th><th className="text-right w-36">Subtotal</th><th className="w-10"></th></tr></thead>
                <tbody>
                  ...
                  <tr className="font-semibold bg-muted">
                    <td colSpan={3} className="text-right">TOTAL:</td>
                    <td className="text-right text-lg">{formatGs(total)}</td>
                    <td></td>
                  </tr>
                </tbody>
              </table>
    ...
    <div className="space-y-6" data-testid="compras-view">
      ...
      <Modal isOpen={showDetailModal} onClose={() => setShowDetailModal(false)} title="Detalle de Compra" size="lg">
        {selectedCompra && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div><span className="text-muted-foreground">Proveedor:</span> <span className="font-medium">{selectedCompra.proveedor_nombre}</span></div>
              <div><span className="text-muted-foreground">Fecha:</span> <span className="font-medium">{new Date(selectedCompra.fecha).toLocaleDateString('es-PY')}</span></div>
              {selectedCompra.numero_factura && <div><span className="text-muted-foreground">Factura:</span> <span className="font-medium">{selectedCompra.numero_factura}</span></div>}
            </div>
            <table className="data-table">
              <thead><tr><th>Producto</th><th className="text-center">Cant.</th><th className="text-right">Precio Unit.</th><th className="text-right">Subtotal</th></tr></thead>
              <tbody>
                ...
                <tr className="font-semibold bg-muted">
                  <td colSpan={3} className="text-right">TOTAL:</td>
                  <td className="text-right text-lg">{formatGs(selectedCompra.total)}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editItem ? 'Editar Compra' : 'Nueva Compra'} size="xl">
        <div className="space-y-4">
          ...
          <div className="border border-border rounded-lg p-4">
            ...
            {form.items.length > 0 && (
              <table className="data-table">
                <thead><tr><th>Producto</th><th className="text-center w-24">Cant.</th><th className="text-right w-36">Costo Unit.</th><th className="text-right w-36">Subtotal</th><th className="w-10"></th></tr></thead>
                <tbody>
                  {form.items.map((item, i) => (
                    <tr key={i}>
                      <td>
...
function ClientesView({ user }) {
  ...
  return (
    <div className="space-y-6" data-testid="clientes-view">
      ...
      <div className="bg-white border border-border rounded-md">
        <table className="data-table">
          <thead><tr><th>Nombre</th><th>RUC</th><th>Teléfono</th><th>Ciudad</th><th>Tipo</th><th>Última Venta</th><th className="text-right">Total Comprado</th><th className="text-right">Compras</th><th className="text-center">Acciones</th></tr></thead>
          <tbody>
            {loading ? <tr><td colSpan={9} className="text-center py-8">Cargando...</td></tr>
            : clientes.length === 0 ? <tr><td colSpan={9} className="text-center py-8">Sin clientes</td></tr>
            : clientes.filter(c => !search || c.nombre?.toLowerCase().includes(search.toLowerCase()) || c.ruc?.includes(search) || c.ciudad?.toLowerCase().includes(search.toLowerCase())).map(c => (
              <tr key={c.id}><td className="font-medium">{c.nombre}</td><td>{c.ruc || '-'}</td><td>{c.telefono || '-'}</td><td>{c.ciudad || '-'}</td><td><span className="badge bg-blue-100 text-blue-800">{c.tipo}</span></td><td className="text-sm">{c.ultima_venta_fecha ? new Date(c.ultima_venta_fecha).toLocaleDateString('es-PY') : '-'}</td><td className="text-right font-mono">{c.total_comprado ? formatGs(c.total_comprado) : '-'}</td><td className="text-center">{c.cantidad_compras || 0}</td>
...
const InventarioView = () => {
  ...
  const loadInventario = async () => {
    try {
      ...
      alert('Error: ' + formatApiError(e.response?.data));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleVerDetalle = async (producto) => {
    try {
      const resp = await api.getInventarioDetalle(producto.id);
      setSelectedProducto(resp.data.producto);
  ...
  const totalValor = inventario.reduce((sum, item) => sum + item.valor_stock, 0);

  return (
    <div style={{ padding: '20px' }}>
      <h2 style={{ fontSize: '24px', fontWeight: 'bold', marginBottom: '20px' }}>Inventario</h2>
      
      <div style={{ marginBottom: '20px', padding: '15px', backgroundColor: '#f9f9f9', borderRadius: '8px', border: '1px solid #e0e0e0' }}>
        <div style={{ marginBottom: '10px' }}>Valor Total Inventario: <strong style={{ fontSize: '18px', color: '#E63946' }}>{formatGs(totalValor)}</strong></div>
        <div>Productos Activos: <strong>{inventario.length}</strong></div>
      </div>
      ...
      <input
        ...
        style={{ width: '100%', padding: '10px', marginBottom: '20px', border: '1px solid #ddd', borderRadius: '5px', fontSize: '14px' }}
      />
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', backgroundColor: 'white' }}>
          <thead>
            <tr style={{ backgroundColor: '#f5f5f5', borderBottom: '2px solid #ddd' }}>
              ...
              <th style={{ padding: '12px', textAlign: 'left', fontWeight: 'bold' }}>Variante</th>
              <th style={{ padding: '12px', textAlign: 'right', fontWeight: 'bold' }}>Stock</th>
              <th style={{ padding: '12px', textAlign: 'right', fontWeight: 'bold' }}>Costo Prom.</th>
              <th style={{ padding: '12px', textAlign: 'right', fontWeight: 'bold' }}>Valor Stock</th>
          ...
          <tbody>
            {inventario.map((item) => (
              <tr key={item.id} style={{ borderBottom: '1px solid #eee' }}>
                ...
                <td style={{ padding: '12px' }}>{item.variante || '-'}</td>
                <td style={{ padding: '12px', textAlign: 'right' }}>{item.stock_actual}</td>
                <td style={{ padding: '12px', textAlign: 'right' }}>{formatGs(item.costo_promedio)}</td>
                <td style={{ padding: '12px', textAlign: 'right', fontWeight: 'bold' }}>{formatGs(item.valor_stock)}</td>
                <td style={{ padding: '12px', textAlign: 'center' }}>
                  <button onClick={() => handleVerDetalle(item)} style={{ padding: '6px 12px', cursor: 'pointer', backgroundColor: '#457B9D', color: 'white', border: 'none', borderRadius: '4px', fontSize: '12px' }}>
                    <Eye size={16} style={{ marginRight: '4px' }} /> Ver
                  </button>
                </td>
              </tr>
            ))}
      ...
      {selectedProducto && (
        <div style={{ marginTop: '30px', padding: '20px', border: '1px solid #ddd', borderRadius: '8px', backgroundColor: '#fafafa' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
            <h3 style={{ margin: 0 }}>Historial: {selectedProducto.nombre}</h3>
            <button onClick={() => setSelectedProducto(null)} style={{ padding: '6px 12px', cursor: 'pointer', backgroundColor: '#999', color: 'white', border: 'none', borderRadius: '4px' }}>Cerrar</button>
          </div>
          <table style={{ width: '100%', borderCollapse: 'collapse', backgroundColor: 'white' }}>
            <thead>
              <tr style={{ backgroundColor: '#f5f5f5' }}>
                <th style={{ padding: '10px', textAlign: 'left', fontWeight: 'bold' }}>Fecha</th>
                <th style={{ padding: '10px', textAlign: 'left', fontWeight: 'bold' }}>Tipo</th>
                <th style={{ padding: '10px', textAlign: 'right', fontWeight: 'bold' }}>Cantidad</th>
                <th style={{ padding: '10px', textAlign: 'left', fontWeight: 'bold' }}>Referencia</th>
              </tr>
            </thead>
            <tbody>
              {movimientos.map((mov, idx) => (
                <tr key={idx} style={{ borderBottom: '1px solid #eee' }}>
                  <td style={{ padding: '10px' }}>{mov.fecha ? mov.fecha.substring(0, 10) : ''}</td>
                  <td style={{ padding: '10px' }}>{mov.tipo}</td>
                  <td style={{ padding: '10px', textAlign: 'right' }}>{mov.cantidad}</td>
                  <td style={{ padding: '10px' }}>{mov.referencia_tipo || '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
