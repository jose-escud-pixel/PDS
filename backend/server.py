...
def get_clientes(request: Request, search: Optional[str] = None):
    ...
    # Add commercial stats to each client
    for cliente in clientes:
        ...
        cliente["ultima_venta_monto"] = ultima_venta.get("total", 0) if ultima_venta else 0
        cliente["ultimo_producto"] = (
            ultima_venta.get("items", [{}])[0].get("nombre", "") + 
            (f" +{len(ultima_venta.get('items', [])) - 1} más" if len(ultima_venta.get("items", [])) > 1 else "")
            if ultima_venta else ""
        )
        
        # Calculate totals
        ventas_cliente = list(ventas_col.find({"cliente_id": cliente_id}))
        cliente["total_comprado"] = sum(v.get("total", 0) for v in ventas_cliente)
        cliente["cantidad_compras"] = len(ventas_cliente)
...
def get_inventario(request: Request, skip: int = 0, limit: int = 100, search: str = None):
    ...
    for p in productos:
        ...
        for m in movements:
            if m["tipo"] == "entrada":
                stock_calc += m["cantidad"]
                valor_movimientos += m.get("valor_movimiento", 0)
            else:
                stock_calc -= m["cantidad"]
        
        # Calculate weighted average cost
        costo_promedio = p.get("costo", 0)
        if movements and valor_movimientos > 0 and stock_calc > 0:
            costo_promedio = valor_movimientos / stock_calc
        
        # Calculate suggested price
        margen = p.get("margen", 15)
        precio_sugerido = costo_promedio * (1 + margen/100)
        ...
        inventario.append({
            ...
            "precio_venta": p.get("precio_con_iva", 0),
            "precio_sugerido": precio_sugerido,
            "valor_stock": stock_calc * costo_promedio
        })
